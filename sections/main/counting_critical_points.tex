...
\begin{corollary}[Critical Point Breakdown]
	...
	matrix \(H\).
	Then we can define the point process of critical points with index
	\(\alpha\) as
	\[
		\pp^\alpha(\Vol, L)
		:= \pp(\Vol, L\cap \real\times\negEV^{-1}(\alpha)).
	\]
	In particular for \(\alpha=0\) we have the height point process of minimas\footnote{
	...
	The sets \(\negEV^{-1}(\alpha)\) are disjoint and its union includes all
	symmetric matrices. And since the hessian of a random field is almost surely
	symmetric, this represents a decomposition of the critical point process,
	i.e.
	\[
		\pp(\Vol, L) = \sum_{k=0}^\dims \pp^{\frac{k}\dims}(\Vol, L).
	\]
	This decomposition translates to the intensity measures of marks
	\[
		\intensityMeas^\alpha(\Vol, L)
		= \E[\pp^\alpha(\Vol, L)]
		\overset{\text{Thm.}~\ref{thm: kac-rice formula}}=
		|\Vol| \markIntMeas^\alpha(L).
	\]
	Now for a random point we would like to talk about its mark distribution. If
	\(\pp\) is a finite measure (finite set), then picking a random critical
	point uniformly is equivalent to sampling \((T,\Mark)=(T,\rf_T,H_T)\sim\pp\).
	Where we rescale \(\pp\) to be a probability measure. But of course, \(\pp\)
	will generally not be finite in \(\Vol\) as its intensity is proportional to
	the lebesgue measure. So let us look at the point process restricted to
	\(\Vol\) first. Asking for the probability of \(\Mark\) in \(L\) then results
	in
	\[
		\Pr_\Mark(L)
		=\frac{\E[\pp(\Vol, L)]}{\E[\pp(\Vol, \real^{1+\dims^2})]}
		= \frac{|\Vol|\markIntMeas(L)}{|\Vol|\markIntMeas(\real^{1+\dims^2})}
		= \frac{\markIntMeas(L)}{\markIntMeas(\real^{1+\dims^2})}
	\]
	As this distribution is independent from \(\Vol\), we simply call
	\(\Pr_\Mark\) the \textbf{distribution of marks}. If we define the intensity
	as
	\[
		\intensity := \markIntMeas(\real\times\real^{\dims\times\dims})
		\overset{\eqref{eq: intensity meas of marks}}= \E[|\det(H)|]\varphi_\rf(0),
	\]
	then we can write
	\[
		\intensityMeas(\Vol, L)
		= \intensity |\Vol| \Pr_\Mark(L)
		= \intensity |\Vol| \sum_{k=1}^\dims
		\Pr(\negEV(H_T) = \tfrac{k}\dims)\Pr(M\in L\mid \negEV(H_T)=\tfrac{k}\dims),
	\]
	where
	\[
 		\Pr(\negEV(H_T) = \alpha)
