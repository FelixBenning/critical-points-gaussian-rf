...
\begin{enumerate}
	...
	In other words: \(\tilde{H} = (\tilde{H}_{ij})\) is a (scaled) Gaussian
	Orthogonal Ensemble (GOE)\footnote{
		...
		\begin{equation}
			\label{eq: GOE ev density constant}
			(c^\evGOE_{\dims,a})^{-1}
...
GOE shifted by an independent shift \(\shift\) consisting of the current height
\(\rf\) and an additional random component \(\epsilon\).

\subsection{\texorpdfstring{\(\evH\mid\rf\)}{η|Z}}
...
\subsection{\texorpdfstring{\(\rf\mid\evH\)}{Z|η}}
